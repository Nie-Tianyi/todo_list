...
pub fn PageNoteFound(segments: Vec<String>) -> Element {
    ...
    rsx! {
        div { class: "flex flex-col items-center justify-center min-h-screen bg-gray-50 px-4",
            ...
            div { class: "absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-blue-100 rounded-full blur-3xl opacity-50 -z-10" }

            // 404 large text
            h1 { class: "text-9xl font-extrabold text-gray-200 select-none",
                "404"
            }

            // Main heading
            h2 { class: "mt-4 text-3xl font-bold text-gray-800 text-center",
                "Page Not Found"
            }

            // Description
            p { class: "mt-3 text-gray-500 text-center max-w-md leading-relaxed",
